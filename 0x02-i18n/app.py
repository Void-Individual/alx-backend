...
def get_timezone():
    """Function to set a timezone"""

    timezone = request.args.get('timezone')
    url_tz = None
    try:
        url_tz = pytz.timezone(timezone)
    except pytz.exceptions.UnknownTimeZoneError:
        user = g.user
        if user:
            url_tz = user.get('timezone', None)
            try:
                url_tz = pytz.timezone(timezone)
            except pytz.exceptions.UnknownTimeZoneError:
                url_tz = None
...
def welcome():
    ...
    tz = datetime.now()
    zone = get_timezone()
    time = tz.astimezone(zone).strftime("%B %d, %Y, %I:%M:%S %p")
    if g.user:
        return render_template('index.html', user=g.user, time=time)
    return render_template('index.html', user=None, time=time)
